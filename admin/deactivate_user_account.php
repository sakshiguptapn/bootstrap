  ...
  if (mysqli_num_rows($result_sql_user_pass_cheack) > 0) {
    while ($row = mysqli_fetch_assoc($result_sql_user_pass_cheack)) {
      ...
      if (mysqli_query($conn, $sql_otp_create)) {
        echo "<script>window.location.href='$hostname/admin/email_sender_files/deactivate_user_account_otp_sender.php'</script>";
      } else {
              ...
              <div class="card mb-3">
                <div class="card-body">
                  <div class="pt-4 pb-2">
                    ...
                    <p class="text-center small">Enter your email & password to Deactivate Account</p>
                  </div>
                  <form action="<?php $_SERVER['PHP_SELF'] ?>" method="POST" class="row g-3 needs-validation" novalidate autocomplete="off">
                    ...
                    <div class="col-12">
                      <button class="btn btn-danger w-100" type="submit" name='login'>Deactivate</button>
                    </div>
                    <div class="col-12">
