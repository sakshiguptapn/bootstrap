...
<?php
session_start();
if (!isset($_GET['id'], $_GET['team_id'])) {
  echo "<script>window.location.href='$hostname/admin/team_member_management_read.php'</script>";
}
$id = $_GET['id'];
...
if (isset($_POST['login'])) {
  ...
  if (mysqli_num_rows($result_sql_user_pass_cheack) > 0) {
    ...
    $sql_user_course_record = "UPDATE team_member SET active_record ='No' WHERE tm_id = '{$id}'";
...
<?php
      echo "<script>window.location.href='$hostname/admin/team_member_management_read.php?id=$team_id'</script>";
    } else {
