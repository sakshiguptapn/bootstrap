    ...
    <div class="pagetitle">
      ...
      <nav>
        <ol class="breadcrumb">
          ...
          <li class="breadcrumb-item active">Modify Team Details</li>
        </ol>
      </nav>
    </div><!-- End Page Title -->
    <section class="section">
      <div class="row">
        <div class="col-lg-12">
          <div class="card">
            <div class="card-body">
              <h5 class="card-title">Modify Team Details</h5>
              <!-- PHP Code for Insert Course Record -->
              <?php
              if (isset($_POST['save'])) {
                try {
                  $category_name = mysqli_real_escape_string($conn, $_POST['category_name']);
                  $sql_update_category = "UPDATE team SET team_name = '{$category_name}' WHERE team_id = '{$id}'";
              ...
              ?>
                  ...
                  ?>
                    <script>
                      alert('Team is not Update !!')
                    </script>
              <?php
                    throw new Exception("Team is not Update.");
                  }
                } catch (\Throwable $th) {
              ...
              <?php
              $sql_fetch_all_category = "SELECT team_name FROM team WHERE team_id = '{$id}'";
                  ...
                  <form class="row g-3" action="<?php $_SERVER['PHP_SELF'] ?>" method="POST" enctype="multipart/form-data" autocomplete="off">
                    <div class="col-md-12">
                      <div class="form-floating mt-3">
                        <input type="text" class="form-control" id="category_name" placeholder="Team Name" name='category_name' value="<?php echo $row_sql_fetch_all_category['team_name']; ?>">
                        <label for="category_name">Team Name</label>
                      </div>
                    </div>
